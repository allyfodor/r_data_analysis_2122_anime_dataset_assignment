...
He wanted to see how the number of people scoring an anime affects its overall score, but also considered a few other variables that might add significantly to the outcome.

So the predictors were:

* Members (how many users saved an anime on their list)
...
```{r}
score_by_members_favourites_scored_by_lm = lm(score ~  members + favorites + scored_by, data = anime_data_distinct_by_name)
```

### Assumptions testing ###

```{r}
check_model(score_by_members_favourites_scored_by_lm,check="linearity")
```

> According to the plot, there might be a problem with the assumption of Linearity.

```{r}
check_model(score_by_members_favourites_scored_by_lm,check="homogeneity")
```

> According to the plot, there might be a problem with the assumption of homoskedasticity, since the residuals are not quite randomly scattered, therefore they do not provide a relatively even distribution,

```{r}
check_model(score_by_members_favourites_scored_by_lm,check="vif")
```

> According to the plot, the VIF statistics are not causing issues, since they are all smaller than 5.

```{r}
check_model(score_by_members_favourites_scored_by_lm,check="outliers")
```

> According to the plot, there are no relevant outliers in the dataset.
```{r}
check_model(score_by_members_favourites_scored_by_lm,check=c("qq","normality"))
```

> According to the plot,the normality of residuals is met. There is only a small deviation from the reference. On the first plot, the residuals do not perfectly fit to the reference, and second plot shows, that our data is slightly left skewed.

```{r}
summary(score_by_members_favourites_scored_by_lm)
```
### Regression analysis ###
The F test indicates that our model provides a better fit than the baseline model (F(3, 13512)= 1130, p<0.001).
According to the analysis, 20% of the dependent variable (Overall scores of anime) can be explained by the independent variables (Adj.R^2=0.2).

### The predictors ###
There is a 2.053e-05 increase in the score of an anime on MyAnimeList for each extra member saving it on their list (t=32.80, B=2.053e-05, p<0.001).
There is a -5.621e-05 decrease in the score of an anime on MyAnimeList for each extra member marking it as their favorite (t=-12.16, B=-5.621e-05, p<0,001).
There is a -2.499e-05 decrease in the score of an anime on MyAnimeList for each extra member scoring it (t=-22.08, B=-2.499e-05, p<0.001).

**So, the number of members scoring an anime on MyAnimeList has a significant negative (but only a minor) effect on the overall score on the anime, after controlling the number of the members interacting with the product in other ways (such as saving it on their list or marking it as their favorite)**
...
> There might be some problems with the linearity since the there is a curve on the plot.

```{r}
check_model(score_by_source_and_rating_lm,check="homogeneity")
```

> According to the plot, there might be a problem with the assumption of homoskedasticity, since the 
> residuals are not randomly scattered, therefore they do not provide a relatively even distribution.
...
> According to the plot, the assumption of the normality of residuals is met. There is only a small deviation from the reference. 
> On the first plot, the residuals do not perfectly fit to the reference line, but there is only an insignificant difference, and the second plot shows, that our data is slightly left skewed.
...
### The predictors ###
If an anime is an original one, its score is 0.56 less than a manga adaptation (t=-17.639, B=-0.56, p<0.0.001).
The rating of anime was not significant (t=1.503, B=0.06, p=0.133).
